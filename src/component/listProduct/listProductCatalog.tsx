    ...
    <>
         {
            products ? (
                products
                .filter((product) => product.product_category === 'Catalog')
                .slice(0, 8)
                .map((product, index) => (
                <Link to={`/detail-product/${product.product_id}`} className='relative overlow-hidden w-[25%] h-[50%] cursor-pointer hover:brightness-[96%] flex-col border p-3 border-slate-300 bg-white overflow-hidden flex'>
                    <div key={index} className='relative overlow-hidden w-full h-full cursor-pointer hover:brightness-[96%] flex-col bg-white overflow-hidden flex'>
